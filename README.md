...
| | |
|---|---|
| 🛡️ **[aegis](https://github.com/rhinolands/aegis)** · *flagship* | **Agent governance gateway: running code, not a diagram.** One enforcement point for agent ingress, A2A delegation, MCP tool egress and LLM calls. **OPA/Rego deny-by-default compiled to WASM** and evaluated in-process · least-privilege credential store, so an agent never holds the backend token · fail-closed on every path · **tamper-evident hash-chained audit** that catches edits *and* deletions even when the database trigger is bypassed · crypto-shredding for GDPR erasure without breaking the chain. **[THREAT_MODEL.md](https://github.com/rhinolands/aegis/blob/main/THREAT_MODEL.md)** maps every control to the OWASP LLM Top 10 and the agentic threat model, with explicit non-goals. TypeScript + Postgres, Apache-2.0. *v0.1 in progress: 48 tests, TDD, every commit reviewed.* |
